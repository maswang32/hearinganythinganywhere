...
# Mod for multiprocessing

def trace_and_save_wrapper(params):
    i, args_dict = params
    if i < args_dict["start_from"]:
        return
    print(i, flush=True)

    from rooms import dataset
    D = dataset.dataLoader(args_dict["dataset"])

    reflections, transmissions, delays, start_directions, end_directions = (
        get_reflections_transmissions_and_delays(
            source=D.speaker_xyz,
            dest=D.xyzs[i],
            surfaces=D.all_surfaces,
            speed_of_sound=D.speed_of_sound,
            max_order=args_dict["max_order"],
            parallel_surface_pairs=D.parallel_surface_pairs,
            max_axial_order=args_dict["max_axial_order"]
        )
    )

    save_dir = args_dict["save_dir"]
    np.save(os.path.join(save_dir, f"reflections/{i}.npy"), np.array(reflections, dtype=object))
    np.save(os.path.join(save_dir, f"transmissions/{i}.npy"), np.array(transmissions, dtype=object))
    np.save(os.path.join(save_dir, f"delays/{i}.npy"), delays)
    np.save(os.path.join(save_dir, f"starts/{i}.npy"), start_directions)
    np.save(os.path.join(save_dir, f"ends/{i}.npy"), end_directions)
    
    del reflections, transmissions, delays, start_directions, end_directions
    gc.collect()
    
if __name__=="__main__":
    
    mp.set_start_method("fork", force=True) # change to "spawn" when run under Windows or Mac
    ...
    if not args.test:
        ...
        with mp.Pool(processes=MAX_PROCESSES) as pool:
            pool.map(trace_and_save_wrapper, [(i, args_dict) for i in indices_to_compute], chunksize=1)
    else:
